...
def prepare_directories_and_logger(hparams,
                                   ...
                                   output_directory,
                                   rank,
                                   inference_text, inference_english_text,
                                   logging_num):
    ...
    if rank == 0:
        ...
        korean_inputs = text_to_sequence(inference_text, hparams.text_cleaners, arpabet_dict, language=1)
        english_inputs = text_to_sequence(inference_english_text, hparams.text_cleaners, arpabet_dict, language=2)
        space = text_to_sequence(' ', hparams.text_cleaners, arpabet_dict, language=2)[0]
        if len(korean_inputs) > len(english_inputs):
            english_inputs += [space] * (len(korean_inputs) - len(english_inputs))
        else:
            korean_inputs += [space] * (len(english_inputs) - len(korean_inputs))
        
        inputs = torch.cat([torch.LongTensor(korean_inputs)[None, :].repeat(logging_num//2, 1),
                            torch.LongTensor(english_inputs)[None, :].repeat(logging_num-logging_num//2, 1)],
                            dim=0)
        inference_english_text
        speaker_ids = torch.zeros(logging_num).long()
        emotion_ids = torch.zeros(logging_num).long()
        
        x_inference = (inputs.cuda(), speaker_ids.cuda(), emotion_ids.cuda())
        
        logger = Tacotron2Logger(prj_name, run_name, resume, inference_text, inference_english_text,
                                 hparams.n_mel_channels, logging_num,
                                 hparams.speakers_name, hparams.emotions_name, hparams.speakers_index, hparams.emotions_index)
    else:
        logger = None
        x_inference = None
...
def wav_logging_select(rank, x_inference, style_logging_len, speakers_index, emotions_index, logging_num):
    if rank == 0:
        inputs, speaker_ids, emotion_ids = x_inference
        
        for i in range(logging_num):
            speaker_ids[i] = speakers_index[random.randint(0, style_logging_len[0] - 1)]
            emotion_ids[i] = emotions_index[random.randint(0, style_logging_len[1] - 1)]
                
        x_inference = (inputs, speaker_ids, emotion_ids)
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            val_log_loss[0] += log_loss[0]
            val_log_loss[1] += log_loss[1]
            
        val_loss = val_loss / (i + 1)
        val_log_loss[0] = log_loss[0] / (i + 1)
        val_log_loss[1] = log_loss[1] / (i + 1)
        
    if rank == 0:
        with torch.no_grad():
            y_inference = model.inference(x_inference)
        logger.log_validation(val_loss, val_log_loss, y, y_pred, x_inference, y_inference, iteration, sample_rate, epoch)
...
def train(prj_name, run_name, resume,
          ...
          inference_text, inference_english_text, logging_num, hparams):
    """
    ...
    logger, x_inference = prepare_directories_and_logger(hparams,
                                                         ...
                                                         inference_text, inference_english_text,
                                                         logging_num)
    style_logging_len = (len(hparams.speakers_name), len(hparams.emotions_name))
    ...
    model.train()
    speakers_index = hparams.speakers_index
    emotions_index = hparams.emotions_index
    is_overflow = False
    # ================ MAIN TRAINNIG LOOP! ===================
    for epoch in range(epoch_offset, hparams.epochs):
        ...
        # 전체 GST 계산
        # GST 분포 계산 후 nearest neighbor
        # outlier 인 경우, 자신 활용
        # outlier 기준 거리 분포 2 시그마?
        # 파일명 연결
        # train_loader
        for i, batch in enumerate(train_loader):
            ...
            if not hparams.fp16_run and is_overflow:
                optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate,
                                 weight_decay=hparams.weight_decay)
                duration = time.perf_counter() - start
                print("Train overflow loss {} {:.6f} {:.2f}s/it".format(
            ...
            if not is_overflow and (iteration % hparams.iters_per_checkpoint == 0):
                validate(model, criterion, valset, iteration,
                        ...
                        wav_logging_select(rank, x_inference, style_logging_len, speakers_index, emotions_index, logging_num))
                if rank == 0:
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--prj_name', default='201023_tacotron2', type=str,
                        help='give a project name for this running')
    parser.add_argument('--run_name', default='nogst',type=str,
    ...
    parser.add_argument('-o', '--output_directory', type=str,
                        default="outdir/201027_tacotron2_nogst",
                        help='directory to save checkpoints')
    parser.add_argument('-c', '--checkpoint_path', type=str,
                        default="outdir/201024_tacotron2_VC_ratio_10s/checkpoint_85000",
                        required=False,
    ...
    parser.add_argument('--inference_english_text', type=str,
                        default='Hello, I am a speech synthesizer.',
                         help='Text for inference')
    ...
    train(args.prj_name, args.run_name, args.wandb_resume,
          ...
          args.inference_text, args.inference_english_text, args.logging_num, hparams)
