...
class Tacotron2Logger():
    def __init__(self, prj_name, run_name, resume, inference_text, inference_english_text, n_mel_channels,
                 logging_num, speakers_name, emotions_name, speakers_index, emotions_index):
        ...
        self.inference_english_text = inference_english_text
        self.speakers_name = speakers_name
        self.emotions_name = emotions_name
        
        self.speakers_index = speakers_index
        self.emotions_index = emotions_index
        
        self.silence = self.mel2wav(torch.zeros(n_mel_channels, 3).cuda()) # actually real silence is -13.875
    ...
    def log_training(self, reduced_loss, log_loss, y_pred, grad_norm, learning_rate, duration,
        ...
        _, _, gate_outputs, alignments = y_pred
    ...
    def log_validation(self, reduced_loss, log_loss, y, y_pred, x_inference, y_inference, iteration, sample_rate, epoch):
        ...
        if mel_len[idx] < 3:
            np_wav_target = self.silence
            np_wav = self.silence
        else:
            mel_target = mel_targets[idx, :, :mel_len[idx]]
            np_wav_target = self.mel2wav(mel_target)
            mel = mel_outputs[idx, :, :mel_len[idx]]
            np_wav = self.mel2wav(mel)
        wandb_log['audio/validation_target'] = wandb.Audio(np_wav_target.astype(np.float32), sample_rate=sample_rate)
        wandb_log['audio/validation'] = wandb.Audio(np_wav.astype(np.float32), sample_rate=sample_rate)
        
        # inference
        _, speaker_ids, emotion_ids = x_inference
        ...
        for i in range(self.logging_num):
            ...
            log_name = 'audio/inference_' + str(i) +'_'+ self.inference_text + '_' + self.inference_english_text
            
            caption = self.speakers_name[self.speakers_index.index(speaker_ids[i])] + '_' + self.emotions_name[self.emotions_index.index(emotion_ids[i])]
